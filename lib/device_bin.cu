...
void bin_gpu_stream(unsigned short *d_input, float *d_output, int nchans, int nsamp, cudaStream_t stream)
{
	...
	bin<<<num_blocks, threads_per_block, 0, stream>>>(d_input, d_output, nsamp);
	//	getLastCudaError("Kernel execution failed");
	...
	swap<<<swap_num_blocks, swap_threads_per_block, 0, stream>>>(d_input, d_output, nchans, nsamp);
	//cudaDeviceSynchronize();
	cudaStreamSynchronize(stream);
	// end_t = clock();
	// double time = double ( end_t - start_t ) / CLOCKS_PER_SEC;
	...
	cudaMemsetAsync(d_output, 0, nchans * nsamp * sizeof(float), stream);
	//cudaMemcpy(input_buffer, bin_buffer, sizeof(float)*nchans*(nsamp/2), cudaMemcpyDeviceToDevice);
	//	getLastCudaError("Kernel execution failed");
}
