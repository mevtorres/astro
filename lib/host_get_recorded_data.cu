...
void get_recorded_data(FILE ***fp, int multi_file, int nsamp, int nchans, int nbits, unsigned short **input_buffer, size_t *inputsize)
{
	...
	if (nbits == 8)
	{
		...
		for(int counter = 0; counter < multi_file; counter++) {
			...
			while (!feof((*fp)[counter]))
			{
				...
				if (fread(temp_buffer, sizeof(unsigned char), nchans, (*fp)[counter]) != nchans)
					break;
