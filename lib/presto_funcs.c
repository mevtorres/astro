...
cufftComplex *presto_gen_w_response(int numbetween, double z,
  ...
  /*  This version uses zero-padding to get the "numbetween"           */
{
    ...
    if (fabs(w) < 1E-4) {
	    response = presto_gen_z_response(z, numkern, numbetween);
        return response;
    }
    ...
    data = (float*) malloc(num_pts_wdat * numbetween * sizeof(float));
    ...
    /* FFT the data */
    fftwf_plan rffplan = fftwf_plan_dft_r2c_1d(num_pts_wdat, data,
					       (fftwf_complex*) data, FFTW_FORWARD);
