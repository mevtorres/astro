...
void dedisperse_stream(int i, int t_processed, int *inBin, float *dmshifts, unsigned short *d_input, float *d_output, int nchans, int nsamp, int maxshift, float *tsamp, float *dm_low, float *dm_high, float *dm_step, int *ndms, int nbits, int failsafe, cudaStream_t stream)
{
	...
	if (failsafe == 0)
	{
		if(nbits == 16 || nbits == 32)
		{
			...
			if (( ( SDIVINT - 1 ) + ( ( SDIVINDM - 1 ) * SDIVINT ) - 1 ) > lineshift)
			{
				...
				cudaDeviceSetSharedMemConfig (cudaSharedMemBankSizeFourByte);
				cudaFuncSetCacheConfig(shared_dedisperse_kernel_16, cudaFuncCachePreferShared);
				shared_dedisperse_kernel_16<<<num_blocks, threads_per_block, 0, stream>>>(inBin[i], d_input, d_output, (float) ( startdm / ( *tsamp ) ), (float) ( dm_step[i] / ( *tsamp ) ));
			}
			else
			{
				printf("\nERROR: smem line length is too short.\nRun the auto tuner again!\n");
		...
		else
		{
			...
			if (( ( SDIVINT - 1 ) + ( ( SDIVINDM - 1 ) * SDIVINT ) - 1 ) > lineshift)
			{
				...
				cudaDeviceSetSharedMemConfig (cudaSharedMemBankSizeFourByte);
				cudaFuncSetCacheConfig(shared_dedisperse_kernel, cudaFuncCachePreferShared);
				shared_dedisperse_kernel<<<num_blocks, threads_per_block, 0, stream>>>(inBin[i], d_input, d_output, (float) ( startdm / ( *tsamp ) ), (float) ( dm_step[i] / ( *tsamp ) ));
			}
			else
			{
				printf("\nERROR: smem line length is too short.\nRun the auto tuner again!\n");
	...
	if(failsafe != 0)
	{
			...
			cudaFuncSetCacheConfig(cache_dedisperse_kernel, cudaFuncCachePreferL1);
			cache_dedisperse_kernel<<<num_blocks, threads_per_block, 0, stream>>>(inBin[i], d_input, d_output, (float) ( startdm / ( *tsamp ) ), (float) ( dm_step[i] / ( *tsamp ) ));
	}
}
