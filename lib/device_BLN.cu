...
int BLN_stream(float *d_input, float *d_MSD, int CellDim_x, int CellDim_y, int nDMs, int nTimesamples, int offset, float multiplier, cudaStream_t stream){
	//---------> Task specific
	int GridSize_x, GridSize_y, x_steps, y_steps, nThreads;
	GridSize_x=(nTimesamples-offset)/CellDim_x;
	GridSize_y=nDMs/CellDim_y;
	...
	//---------> MSD
	BLN_init();
	BLN_MSD_GPU_grid<<<gridSize,blockSize,nThreads*8,stream>>>(d_input, d_output, x_steps, y_steps, nTimesamples, 0);
