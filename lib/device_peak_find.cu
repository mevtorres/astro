...
void PEAK_FIND_stream(float *d_output_SNR, ushort *d_output_taps, float *d_peak_list, int nDMs, int nTimesamples, float threshold, int max_peak_size, int *gmem_peak_pos, int shift, std::vector<PulseDetection_plan> *PD_plan, int max_iteration, cudaStream_t stream){
	int decimated_timesamples, local_offset;
	...
	for(int f=0; f<max_iteration; f++){
		...
		if( (decimated_timesamples-local_offset-1)>0 ){
			...
			dilate_peak_find<<<gridSize, blockDim, 0, stream>>>(&d_output_SNR[nDMs*PD_plan->operator[](f).output_shift], &d_output_taps[nDMs*PD_plan->operator[](f).output_shift], d_peak_list, decimated_timesamples, nDMs, local_offset, threshold, max_peak_size, gmem_peak_pos, shift, (1<<f));
		}
	}
}
