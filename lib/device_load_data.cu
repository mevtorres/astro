...
void load_data_stream(int i, int *inBin, unsigned short *device_pointer, unsigned short *host_pointer, int t_processed, int maxshift, int nchans, float *dmshifts, cudaStream_t stream)
{
	...
	if (i == -1)
	{
		int length = ( t_processed + maxshift );
		size_t size = nchans * length * sizeof(unsigned short);
		cudaMemcpyToSymbolAsync(dm_shifts, dmshifts, nchans * sizeof(float), 0, cudaMemcpyDefault, stream);
		cudaMemcpyAsync(device_pointer, host_pointer, size, cudaMemcpyHostToDevice, stream);
		cudaMemcpyToSymbolAsync(i_nchans, &nchans, sizeof(int), 0, cudaMemcpyDefault, stream);
		cudaMemcpyToSymbolAsync(i_nsamp, &length, sizeof(int), 0, cudaMemcpyDefault, stream);
		cudaMemcpyToSymbolAsync(i_t_processed_s, &t_processed, sizeof(int), 0, cudaMemcpyDefault, stream);
	}
	else if (i > 0)
	{
		int length = ( t_processed + maxshift );
	...
	for (int f = 0; f <= PD_MAXTAPS; f++)
		h_sqrt_taps[f] = (float) sqrt((double) f);
	cudaMemcpyToSymbolAsync(c_sqrt_taps, h_sqrt_taps, ( PD_MAXTAPS + 1 ) * sizeof(float), 0, cudaMemcpyDefault, stream);
}
